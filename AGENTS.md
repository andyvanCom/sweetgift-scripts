...
- `sweetgift-proposal.js` owns the verified-email PDF proposal dialog on the two composition selectors and the gift quiz. Producers publish only `source`, up to 12 catalog `productKeys`, and the target DOM mount through `sg:proposal-selection`; all names, prices, photos and composition are reloaded server-side. The JWT-protected `proposal-mailer` sends both verification codes and PDFs through the same protected Yandex SMTP secrets and sender used by the existing selector/admin mail flows. The gateway validates the legacy anon JWT and the function independently validates `x-sweetgift-service-key` against a pinned SHA-256 fingerprint; rotate the key and fingerprint together, and keep SMTP/service keys out of this public repository and all browser code.
- The Tilda gift quiz must load its browser core from the public `sweetgift-scripts` CDN asset `gift-quiz-core.v1.js`, not from `app.sweetgift.ru`. Resolve the core as a sibling of the executing quiz script so both assets use the same immutable Git commit; never combine a commit-pinned module with a mutable `@main` core URL. This keeps the selector UI available during an application-server or reverse-proxy outage. Increment the quiz manifest version after frontend changes and the fallback core query version after core changes.
- During a quiz RPC, show an approximate countdown and rotate messages derived only from filters the customer actually selected. These messages describe local filtering intent, not fabricated server-side completion percentages; retain the bounded timeout and explicit retry action.
