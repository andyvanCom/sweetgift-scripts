...
Вопросы, варианты, ветвление, обязательность шагов, значения пропуска, бюджет,
прогресс, URL и подготовка `p_answers` находятся в одном модуле
`packages/gift-quiz-core` репозитория `sweetgift-order-status-bot`.
`sweetgift-gift-quiz.js` содержит только адаптер интерфейса Tilda и загружает
браузерную сборку модуля с
`https://app.sweetgift.ru/gift-quiz-core.v1.js`.

Сначала должна быть развёрнута ветка мини-приложения, чтобы файл общего модуля
стал доступен, и только затем обновлён скрипт сайта. Для стенда или локальной
проверки URL сборки можно задать атрибутом `data-quiz-core` на теге скрипта.

Алгоритм рекомендаций не переносился во frontend: и сайт, и мини-приложение
по-прежнему вызывают одну функцию
`public.get_gift_quiz_recommendations(p_answers jsonb, p_limit integer)`.
