...
async function getDashboard(days: number) {
  ...
  const [
    ...
    await Promise.all([
      ...
      supabase.from("admin_daily_metrics").select(
        "snapshot_date,articles_active,article_views_total,database_bytes",
...
const actions: Record<string, { name?: string; query?: string; rpc?: string }> = {
  ...
  articleProducts: { rpc: "request_article_product_cache_refresh" },
...
.charts{display:grid;grid-template-columns:repeat(3,1fr);gap:13px}.chart svg{width:100%;height:130px;display:block;margin-top:10px}.chart .line{fill:none;stroke:var(--red);stroke-width:3;stroke-linecap:round;stroke-linejoin:round}.chart .area{fill:#d85c7b20}.chart-meta{display:flex;justify-content:space-between;gap:8px}@media(max-width:880px){.charts{grid-template-columns:1fr}}
</style></head><body>
...
<section class="section"><h2>История за 90 дней</h2><div id="history" class="charts"></div></section>
...
function chart(title,rows,key,format=v=>String(v)){if(!rows.length)return '<div class="card chart"><b>'+esc(title)+'</b><div class="sub">История начнёт заполняться раз в сутки</div></div>';const values=rows.map(x=>Number(x[key])||0),min=Math.min(...values),max=Math.max(...values),span=Math.max(1,max-min),points=values.map((v,i)=>((i/Math.max(1,values.length-1)*300).toFixed(1))+','+(110-(v-min)/span*90).toFixed(1)).join(' '),area='0,110 '+points+' 300,110',last=values.at(-1);return '<div class="card chart"><div class="chart-meta"><b>'+esc(title)+'</b><b>'+esc(format(last))+'</b></div><svg viewBox="0 0 300 120" preserveAspectRatio="none"><polygon class="area" points="'+area+'"></polygon><polyline class="line" points="'+points+'"></polyline></svg><div class="chart-meta sub"><span>'+esc(rows[0].snapshot_date)+'</span><span>'+esc(rows.at(-1).snapshot_date)+'</span></div></div>'}
function renderHistory(rows){el("history").innerHTML=chart("Активные статьи",rows,"articles_active")+chart("Просмотры статей",rows,"article_views_total",v=>new Intl.NumberFormat("ru-RU").format(v))+chart("Размер базы",rows,"database_bytes",v=>(v/1048576).toFixed(1)+" МБ")}
function render(d){const o=d.orders,c=d.article_product_cache||{};el("metrics").innerHTML=[["Заказы",o.total],["Выручка",money(o.revenue)],["Средний чек",money(o.average_check)],["Товаров продано",o.items],["Подарки",o.gifts],["С поздравлением",o.messages],["С промокодом",o.promocodes],["Каталог",d.catalog.products+" / "+d.catalog.articles],["Подборки статей",(c.cached_aliases||0)+" / "+(c.cached_products||0)]].map(x=>'<div class="metric card"><div class="sub">'+esc(x[0])+'</div><b>'+esc(x[1])+'</b></div>').join("");renderHistory(d.history||[]);el("jobs").innerHTML=d.pipeline.map(j=>'<tr><td>'+esc(j.job_name)+'</td><td><span class="status '+(j.status==="success"?"success":"")+'"><i class="dot"></i>'+esc(j.status)+'</span></td><td>'+esc(j.processed_count)+'</td><td>'+date(j.finished_at||j.started_at)+'</td></tr>').join("");bars("daily",o.daily,"date");bars("products",o.top_products);bars("categories",o.top_categories);el("orders").innerHTML=o.recent.length?o.recent.map(x=>'<tr><td>…'+esc(x.order_id)+'</td><td>'+date(x.created_at)+'</td><td>'+esc(x.items)+'</td><td>'+money(x.total)+'</td></tr>').join(""):'<tr><td colspan="4" class="sub">Заказов за период нет</td></tr>';el("report").textContent=d.report}
