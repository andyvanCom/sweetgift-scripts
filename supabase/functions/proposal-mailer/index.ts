  ...
  try {
    ...
    try {
      if (type === "code" && /^\d{6}$/.test(code)) {
        await client.send({ from: FROM_EMAIL, to: email, subject: "SweetGift: verification code", html: asciiHtml(`<div style="font:16px/1.5 Arial,sans-serif;color:#261d1f;max-width:560px;margin:auto;padding:28px"><h2>SweetGift</h2><p>Ваш код подтверждения:</p><p style="font-size:32px;font-weight:700;letter-spacing:6px;color:#a9284d">${code}</p><p>Код действует 10 минут. Если вы не запрашивали коммерческое предложение, просто проигнорируйте письмо.</p></div>`) });
      } else if (type === "proposal" && pdfBase64.length > 100) {
        await client.send({ from: FROM_EMAIL, to: email, subject: "SweetGift: commercial proposal", html: asciiHtml(`<div style="font:16px/1.5 Arial,sans-serif;color:#261d1f;max-width:560px;margin:auto;padding:28px"><p>${escapeHtml(name)}, добрый день!</p><p>Во вложении коммерческое предложение с подобранными вариантами SweetGift.</p><p>Если появятся вопросы, ответьте на это письмо — мы на связи.</p></div>`), attachments: [{ filename: "SweetGift-kommercheskoe-predlozhenie.pdf", content: pdfBase64, encoding: "base64", contentType: "application/pdf" }] });
