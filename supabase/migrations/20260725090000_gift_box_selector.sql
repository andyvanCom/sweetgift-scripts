-- Reuse the ingredient selector pipeline for two strictly separated product
-- collections: food baskets and boxed gift sets.

create or replace function public.get_gift_selector_catalog_for(p_collection text)
returns jsonb
language sql
stable
security definer
...
as $$
  with available_products as (
    ...
    where p.available = true
      and case p_collection
        ...
        when 'boxes' then (
          lower(coalesce(p.category_slug, '')) in (
            'novogodnie-nabory',
            'ng-gift-box',
            'gift-nabor',
            'box-s-lomom',
          ...
          or lower(coalesce(p.category_slug, '')) like 'box-%'
        )
        else false
      end
  ),
  ...
  product_ingredient_lists as (
    select product_key, array_agg(ingredient order by ingredient) as ingredients
  ...
  ingredient_counts as (
    select ingredient, count(distinct product_key)::integer as product_count
    from normalized_ingredients
    group by ingredient
  )
  select jsonb_build_object(
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object('name', ingredient, 'product_count', product_count)
        order by ingredient
      )
...
set search_path = public
as $$
  select public.get_gift_selector_catalog_for('baskets');
$$;
...
set search_path = public
as $$
  select public.get_gift_selector_catalog_for('boxes');
$$;

comment on function public.get_gift_selector_catalog_for(text) is
  'Internal shared catalog builder for ingredient selector collections.';
comment on function public.get_gift_selector_catalog() is
  'Returns available gift baskets and Russian ingredient labels.';
comment on function public.get_gift_box_selector_catalog() is
  'Returns available boxed gift sets and Russian ingredient labels.';

revoke all on function public.get_gift_selector_catalog_for(text) from public;
