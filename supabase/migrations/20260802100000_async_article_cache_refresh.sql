-- PostgREST cancels a synchronous cache rebuild before the growing article
-- catalog finishes. Queue manual rebuilds in pg_cron so the admin action can
-- return immediately while PostgreSQL performs the existing safe refresh.
...
comment on function public.request_article_product_cache_refresh() is
  'Queues one self-removing pg_cron cache rebuild and returns without a PostgREST timeout.';
