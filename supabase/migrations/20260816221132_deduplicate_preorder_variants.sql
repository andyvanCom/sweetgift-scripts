...
as $$
  with input as (
    select trim(search_query) as original,
           lower(translate(trim(search_query), 'Ёё', 'Ее')) as normalized
  ),
  candidates as (
    select v.variant_key, p.product_key, v.group_id, v.option_name, v.option_value,
    ...
    where p.available is true
      ...
      and lower(trim(coalesce(v.option_name, ''))) <> 'предзаказ'
      and length(i.original) between 2 and 200
      and (
        v.url = i.original or v.variant_key = i.original or p.product_key = i.original
    ...
    union all
    select p.product_key, p.product_key, null::text, null::text, null::text,
           regexp_replace(p.title, '\s*-\s*(в наличии|[0-9]+\s*(день|дня|дней))\s*$', '', 'i') as title,
           regexp_replace(p.url, '\?editionuid=[0-9]+$', '', 'i') as url,
           p.image, p.price, p.category_slug,
           case
    ...
    where p.available is true
      ...
      and length(i.original) between 2 and 200
      and (
        p.url = i.original or p.product_key = i.original
...
comment on function public.neuromaria_search_products_v2(text, integer) is
  'Bounded NeuroMaria product search. Customer variants remain separate; preorder lead-time offers collapse to one product.';
