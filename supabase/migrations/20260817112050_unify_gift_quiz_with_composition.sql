-- Unify the existing gift quiz with basket/box format and composition filters.
-- Ingredient values are the normalized product_ingredients.tag values already
-- used by the two dedicated composition selector pages.
...
with input as (
 ...
 coalesce(p_answers->>'format','any') format,
...
features as (
 ...
 case when format in ('basket','box') then 10 else 0 end+
 ...
 and (
   format='any'
   or (format='basket' and (
     lower(coalesce(title,'')) like '%корзин%'
     or lower(coalesce(product_key,'')) like '%korzin%'
   ))
   or (format='box' and (
...
result as (
 ...
 case when format='basket' then 'В формате подарочной корзины' when format='box' then 'В формате бокса или набора' end,
 case when jsonb_array_length(ingredients)>0 and not(ingredients ? 'none') then 'Содержит все выбранные ингредиенты' end,
...
comment on function public.get_gift_quiz_recommendations(jsonb,integer) is
  'Ranks available gifts by recipient, budget, style, timing, format and an AND-filter over selected normalized ingredients.';
