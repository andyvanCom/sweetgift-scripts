...
`;
  ...
  function renderTabs(period) {
    ...
    return '<div class="sg-top-articles-tabs">' + tabs.map(function (tab) {
      var code = tab[0];
      var title = tab[1];
      var active = code === period ? ' is-active' : '';

      return '<a class="sg-top-articles-tab' + active + '" href="?period=' + code + '">' +
        escapeHtml(title) +
      '</a>';
    }).join('') + '</div>';
  }

  function render(root, items, period) {
    ...
    html += '<div class="sg-top-articles-grid">';
